Email verification in a Laravel 5.8 app
This app is going to send emails to verify that an email address is real when users sign up. Before their account is active users 
need to click a special link that will verify their account and that they're in control of that email address. 
This is a common web functionality we've all experienced when signing up for new accounts online. The impetus for this project
is that I deployed a Laravel app for people to use and put it behind a login form. Many people used garbage emails to sign up and
access the app. This isn't protected by default with Laravel's auth scaffold, so let's add it real quick!
...
Step 2
Head into the Laravel database migrations files and find the create_users_table migration. 
(Hint: it's in database/migrations.) From there, add two columns to the users table for a token and an is verified field. 
The verified will default to zero for false.
public function up()
{
...
MAIL_PASSWORD=
MAIL_ENCRYPTION=tls
MAIL_FROM_ADDRESS=chuukwuma30@gmail.com
MAIL_FROM_NAME="Innocentr"
For the new values to take effect you may need to run php artisan config:clear
If you are sending emails from Gmail you'll have to turn off additional protections. 
Head to https://www.google.com/settings/security/lesssecureapps and turn off this setting.
Without this, Google will block your Laravel app's sign in attempts.
Step 5
